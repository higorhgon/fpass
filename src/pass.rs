...
impl PassStore {
    ...
    /// Remove um grupo (diretório) sem senhas, usando `pass rm -r`: ao
    /// contrário de `fs::remove_dir`, lida sozinho com qualquer arquivo
    /// oculto remanescente (ex.: um `.gpg-id` de escopo próprio), já que
    /// remove a árvore inteira recursivamente. Só é chamado para grupos que
    /// `Backend::list` já determinou não ter entradas/subgrupos visíveis.
    pub fn rm_group(&self, group: &str) -> Result<(), String> {
        run_pass(&self.root, &["rm", "-r", "-f", group], None)?;
        Ok(())
    }
...
fn parse_entry(content: &str) -> EntryData {
...
fn build_entry_content(data: &EntryData) -> Zeroizing<String> {
